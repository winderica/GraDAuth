const mcl_c = require('./mcl_c.js');
const isBrowser = typeof window !== 'undefined';
const crypto = isBrowser ? window.crypto : require(`cry` + `pto`);
...
const setup = (mod, curveType) => {
    ...
    */
    mcl.mul = (x, y) => {
        if (x instanceof mcl.Fp && y instanceof mcl.Fp) {
        ...
        if (x instanceof mcl.Fp2 && y instanceof mcl.Fp2) {
    ...
    */
    mcl.mulVec = (xVec, yVec) => {
        if (xVec.length == 0) throw new Error('mulVec:zero array');
        if (xVec[0] instanceof mcl.G1 && yVec[0] instanceof mcl.Fr) {
            return _mulVec(mod._mclBnG1_mulVec, xVec, yVec, mcl.G1);
        }
        if (xVec[0] instanceof mcl.G2 && yVec[0] instanceof mcl.Fr) {
            return _mulVec(mod._mclBnG2_mulVec, xVec, yVec, mcl.G2);
        }
    ...
    mcl.pairing = (P, Q) => {
        ...
        throw new Error('mcl.pairing:bad type');
    };
    ...
    mcl.millerLoop = (P, Q) => {
        ...
        throw new Error('mcl.millerLoop:bad type');
    };
    mcl.precomputedMillerLoop = (P, Qcoeff) => {
        if (!(P instanceof mcl.G1 && Qcoeff instanceof mcl.PrecomputedG2)) throw new Error('mcl.precomputedMillerLoop:bad type');
        const e = new mcl.GT();
    ...
    mcl.precomputedMillerLoop2 = (P1, Q1coeff, P2, Q2coeff) => {
        if (!(P1 instanceof mcl.G1 && Q1coeff instanceof mcl.PrecomputedG2 && P2 instanceof mcl.G1 && Q2coeff instanceof mcl.PrecomputedG2)) throw new Error('mcl.precomputedMillerLoop2mixed:bad type');
        const e = new mcl.GT();
    ...
    mcl.precomputedMillerLoop2mixed = (P1, Q1, P2, Q2coeff) => {
        if (!(P1 instanceof mcl.G1 && Q1 instanceof mcl.G2 && P2 instanceof mcl.G1 && Q2coeff instanceof mcl.PrecomputedG2)) throw new Error('mcl.precomputedMillerLoop2mixed:bad type');
        const e = new mcl.GT();
        const P1Pos = P1._allocAndCopy();
