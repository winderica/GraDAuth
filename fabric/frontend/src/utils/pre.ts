...
export class PRE {
    ...
    keyGenInG1(g: G1, sk?: string) {
        const ska = sk ? this.mcl.deserializeHexStrToFr(sk) : this.randomInFr(); // SKa = a is randomly selected from Fr
        const pk = this.mcl.mul(g, ska); // PKa = g^SKa = g^a
        return { sk: ska, pk };
    }
